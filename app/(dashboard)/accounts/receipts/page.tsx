...
const TX_LABELS: Record<string, string> = {
  ReceiptAdvance: "Advance",
  ReceiptBalance: "Balance",
  ReceiptFull: "Full Payment",
  UdharCredit: "Udhar Cleared",
};
...
export default function ReceiptBookPage() {
  ...
  const { data, isLoading } = useQuery<PagedResult<ReceiptEntry>>({
    queryKey: ["receipt-book", modeTab, from, to, accountId, page, pageSize],
    queryFn: async () => (await api.get("/api/accounting/receipt-book", { params })).data,
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["receipt-book"] });
      setPanelOpen(false);
      setEditingId(null);
    },
  });
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
       {/*  <div className="flex items-center gap-2 px-5 py-4" style={{ borderBottom: "1px solid var(--border)" }}>
          <ArrowDownLeft size={14} style={{ color: "var(--color-green)" }} />
          <p className="text-[13px] font-semibold" style={{ color: "var(--text)" }}>Receipt Book</p>
        </div>
 */}
        {isLoading ? (
          <p className="px-5 py-8 text-[12px] text-center" style={{ color: "var(--text3)" }}>Loading…</p>
        ) : entries.length === 0 ? (
          <p className="px-5 py-8 text-[12px] text-center" style={{ color: "var(--text3)" }}>No receipts found for this period.</p>
        ) : (
                  ...
                  <tr key={e.id} style={{ borderBottom: i < entries.length - 1 ? "1px solid var(--border)" : undefined }}>
                   ...
                   {/*  <td className="px-4 py-2.5 whitespace-nowrap">
                      <span className="rounded px-1.5 py-0.5 text-[9px] font-medium"
                        style={{ background: "rgba(72,199,142,0.12)", color: "var(--color-green)" }}>
                    ...
                    <td className="px-4 py-2.5 text-right whitespace-nowrap font-semibold" style={{ color: "var(--color-green)" }}>
                      {formatInr(e.amount)}
                    </td>
                      ...
                      {e.isManual ? (
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              setDeleteTarget({ id: e.id, label: e.accountName || "Receipt" });
                            }}
                            className="rounded-md p-1.5"
      ...
      {(entries.length > 0 || totalPages > 1) && (
        ...
        >
          ...
          <div className="flex items-center gap-3">
           {/*  <p className="text-[13px] font-bold" style={{ color: "var(--color-green)" }}>{formatInr(total)}</p> */}
      ...
      <SlidePanel
        ...
        title={editingId ? "Edit Receipt" : "Add Receipt"}
        subtitle="Manual entry (Order receipts are not editable)"
        width="460px"
      >
        <div className="space-y-3">
          ...
          >
            {editingId ? "Save Changes" : "Save Receipt"}
          </button>
        </div>
      </SlidePanel>
    </div>
